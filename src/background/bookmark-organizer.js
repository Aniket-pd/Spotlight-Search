...
function getRecentBookmarkNodes(limit) {
  return new Promise((resolve, reject) => {
    try {
      chrome.bookmarks.getRecent(limit, (nodes) => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
        }
...
function getBookmarksByIds(ids) {
  return new Promise((resolve, reject) => {
    ...
    try {
      chrome.bookmarks.get(normalizedIds, (nodes) => {
        if (chrome.runtime.lastError) {
          reject(chrome.runtime.lastError);
          return;
        }
...
async function collectRecentBookmarks(limit) {
  ...
  const requestedCount = Math.max(clampedLimit, Math.min(MAX_BOOKMARK_LIMIT * 2, clampedLimit * 2));

  let recent = [];
  try {
    recent = await getRecentBookmarkNodes(requestedCount);
  } catch (error) {
    console.warn("Failed to load recent bookmarks for organizer", error);
  }

  const recentBookmarks = Array.isArray(recent)
    ? recent.filter((node) => node && node.url && normalizeId(node.id) !== null)
    : [];
  ...
  const limited = recentBookmarks.slice(0, clampedLimit);
  ...
  const enqueueParent = (parentId) => {
    if (!parentId && parentId !== "0") {
      return;
    }
    if (scheduledParents.has(parentId)) {
      return;
    }
    scheduledParents.add(parentId);
  ...
  while (pendingParents.size) {
    const chunk = Array.from(pendingParents).slice(0, CHUNK_SIZE);
    for (const id of chunk) {
      pendingParents.delete(id);
    }

    const nodes = await getBookmarksByIds(chunk).catch((error) => {
      console.warn("Failed to load bookmark parents for organizer", error);
      return [];
    });
    ...
    for (const node of nodes) {
      ...
      const id = normalizeId(node.id);
      if ((!id && id !== "0") || processedParents.has(id)) {
        continue;
      }
  ...
  const bookmarks = limited.map((node) => {
    const id = normalizeId(node.id);
    const parentId = normalizeId(node.parentId);
    const title = typeof node.title === "string" && node.title ? node.title : node.url || "Untitled bookmark";
    const dateAdded = typeof node.dateAdded === "number" ? node.dateAdded : null;
    const ancestors = [];
    const visited = new Set();
    let currentId = parentId;

    while ((currentId || currentId === "0") && nodeMap.has(currentId) && !visited.has(currentId)) {
    ...
    const parentTitle = ancestors.length ? ancestors[ancestors.length - 1].title || "" : "";
  ...
  return { bookmarks, folderLookup, nodeMap };
}
